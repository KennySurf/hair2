...
def return_message_classification(user_id: str, message: str) -> str:
   ...
   booking: всё что касается записи — если пользователь явно говорит, что хочет записаться, 
    указывает время, дату, мастера, или формулировки типа "запишите", "подберите мастера", 
    "можно в такое-то время" и т.п. 
    Если пользователь просто упоминает или выбирает услугу без явного желания записи — это не booking.
    ...
    tryon_request: запрос на примерку услуги, например, "хочу примерить", "давайте попробуем", "как будет выглядеть эта прическа". Или клиента спросили - хочет ли он примерить услугу на себе - он ответил да
