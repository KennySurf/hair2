...
def get_tryon_accept_reply(user_id, user_message):
    ...
    if tryon_state == 'getting_a_prompt':
        start_prompt = """
        Сгенерируй промпт для изменения фотографии пользователя, опираясь на его запросы по изменению прически.
        Примечание: промпт не должен менять лицо, позу, фон или другие элементы, он должен касаться только прически.
        Промпт должен быть на английском языке.
        Если пользователь не упоминает изменение прически, верни: не прическа.
        Если информации от пользователя недостаточно для генерации изображения, верни: недостаточно
        Например пользователь сказал что хочет изменить длину, но на сколько не сказал - недостаточно
        Не выводи ничего другого — либо возвращай только промпт на английском, либо не прическа, либо недостаточно
        """
        ...
        if user_prompt == 'не прическа':
            ...
            """
        elif user_prompt == 'недостаточно':
            reply_prompt = """
            Задай клиенту уточняющие вопросы, чтобы дополнить промпт
            """
        else:
            reply_prompt = """
