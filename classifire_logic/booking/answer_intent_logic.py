...
def answer_intent(user_id, user_message):
    ...
    if state == 'idle':
        ...
        prompt = f"""
           ...
           Не спрашивай ничего больше - ни дату, ни время визита, только единственный вопрос - какая услуга интересует?
           """
    ...
    if state == 'get_services':
        ...
        if reply in ('наращивание волос', 'окрашивание волос'):
            reply = send_to_gpt(history_user_messages + [{'role': 'system', 'content': 'сообщи клиенту, что ему нужно сначала записаться на бесплатную консультацию. На ней мы подберём лучшее решение для волос клиента. Спроси, записать ли его? Не говори "понимаю" и не спрашивай время/дату - единственный вопрос: хочет ли он записаться?'}])
        ...
        Если последнее сообщение - хочу записаться или что то типо того - это тоже да.
        Выводи только да или нет
        """
        reply = send_to_gpt([{'role': 'system', 'content': prompt}] + history_user_messages[1:])
        print(reply)
        if reply == 'да':
            ...
            Завершай ответ вопросом, хочет ли он записаться на консультацию?.
            ...
            Заверши : «Хотите записаться на консультацию?»."""
            reply = send_to_gpt(history_user_messages + [{'role': 'system', 'content': prompt}])
            add_message(user_id, 'assistant', reply)
        ...
        prompt = f"""
           Вежливо спроси имя мастера, к которому клиент хочет записаться, дав список мастеров.
        ...
        prompt = f"""
                    ...
                    Если имя выбранного мастера, встречется в нашем перечне -  верни выбранного мастера строго так, как он записан в нашем перечне
                    Если нет, верни False"""

        reply = send_to_gpt(history_user_messages[1:] + [{'role': 'system', 'content': prompt}])
        if reply != 'False':
