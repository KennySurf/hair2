...
def return_booking_intent(user_id, message: str) -> str:
    ...
    Например:
    -Есть ли у вас мастер, к которому хотите записаться, или помочь подобрать?
    -помочь
    ВЫВОД - answer, тк отвечает на вопрос
    
    other: Строго если клиент задаёт вопрос
    Например:
    -Как мне записаться на услугу?
    Вывод - other, тк сам задаёт вопрос

    Верни только один из вариантов: answer или other.
    Если клиент отвечает на заданный ранее вопрос или говорит что хочет записаться - answer, спрашивает что-то - other и никак иначе 
    """

    messages = get_user_messages(user_id)
    classifier_message = messages[1:-1] + [{'role': 'system', 'content': prompt}] + [messages[-1]]
    print(f'nlu_booking: {classifier_message}')
