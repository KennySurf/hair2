...
def get_service_reply(user_id, intent):
    ...
    Не нужно уточнять технические детали или спрашивать, какой именно метод интересует — 
    главное мягко предложить примерку. """
    ...
    Отвечай клиенту на его вопрос, используя информацию из файла, но не говори, что берёшь её из файла, если информации недостаточно, можешь добавлять сам ответ, только руководствуйся ранее писаными правилами.
    Тон: дружелюбный, лёгкий, с заботой, будто общается живой администратор.
    Формулировки делай естественными, иногда вариативными — не повторяй одну и ту же фразу слово в слово.
